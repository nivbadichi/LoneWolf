...
async function getAllEvents(req, res, next) {
  try 
  {
    ...
    // 5. Execute the query
    const events = await eventModel.Event.find(queryFilter);
...
async function getNearbyEvents(req, res, next) {
  try 
  {
    ...
    const now = new Date();
    // NOTE: .populate('hostId', ...) omitted — userModel.js has no User model
    // registered yet, so populate would throw a MissingSchemaError.
    const activeEvents = await eventModel.Event.find({
      endTime: { $gte: now }
    });
...
async function updateEvent(req, res, next) {
  try {
    const event = await eventModel.Event.findById(req.params.id);
